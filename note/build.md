...
`DllPlugin`抽离公共代码，后续打包不打包该代码。`DllReferencePlugin`通过`manifest.json`引用打包好代码

### HappyPack

webpack loader虽然是异步执行，但内部执行任务还是单线程的，`HappyPack`开启子线程并发执行，等子线程处理完毕后再把结果发给主线程，加速打包速度。
