...
* `join(:?string)`：调用每一项的`toString()`方法，并用传入的参数拼接，如果参数为空或者`undefined`，则默认用`,`拼接。
...
arr1.sort(function (a, b) {
	return a - b // <0 第1个数在前，>0 第2个数在前，=0不改变顺序
}) // => [1, 4, 21, 221, 223]
```
...
* `slice(a1:?number, a2:?number)`返回下标从a1到a2(不包含a2)的子数组，**不改变**调用数组。如果不指定a2，则截取到最后一个元素。并且参数为负数时，表示倒数第几个元素的位置。
...
* `splice(a1?:number,a2:?number,...)`，插入或删除元素，返回删除的部分，**会改变**调用数组。第1个参数指定起始位置，第2个参数指定删除的元素个数（可为空），随后为新插入的元素。
